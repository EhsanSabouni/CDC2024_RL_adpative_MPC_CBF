...
class Logger(object):
    def __init__(self, log_dir, use_tb):
        ...
        self._traffic_eval_mg = MetersGroup(log_dir / 'traffic_eval.csv',
                                    formating=COMMON_TRAFFIC_EVAL_FORMAT)
        if use_tb:
    ...
    def log(self, key, value, step):
        assert key.startswith('train') or key.startswith('eval') or key.startswith('traffic')
    ...
    def dump(self, step, ty=None):
        ...
        if ty is None or ty == 'traffic_eval':
            self._traffic_eval_mg.dump(step, 'traffic_eval')
